...
for deceto, contagem in decetos_ordenados:
    if contagem > 1 and len(deceto) > 19:
        # Extract values inside parentheses and remove dashes and numbers
        formatted_deceto = ''.join(c for c in deceto if c.isalpha() or c.isspace())
        print(f"{formatted_deceto.strip()}, Contagem: {contagem}")
